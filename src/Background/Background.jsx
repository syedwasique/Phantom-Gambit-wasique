...
class Z extends d {
  ...
  setColors(e) {
    if (Array.isArray(e) && e.length > 0) {
      const colorObjects = e.map(c => {
        // Convert string color to proper Three.js Color
        if (typeof c === 'string') {
          // Remove alpha from hex if present
          ...
          if (c.startsWith('rgba(')) {
            ...
            return new l(r/255, g/255, b/255);
          }
        }
        return new l(c);
      });
      ...
      for (let idx = 0; idx < this.count; idx++) {
        const colorIndex = idx % colorObjects.length;
        this.setColorAt(idx, colorObjects[colorIndex]);
        if (idx === 0) {
          this.light.color.copy(colorObjects[colorIndex]);
        }
      }
